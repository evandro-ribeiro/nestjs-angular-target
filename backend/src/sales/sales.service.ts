...
export class SalesService {
  ...
  calculateCommissions() {
    ...
    this.data.vendas.forEach((venda) => {
      ...
      if (!report[venda.vendedor]) {
        report[venda.vendedor] = { totalVendas: 0, totalComissao: 0 };
      }

      report[venda.vendedor].totalVendas += venda.valor;
      report[venda.vendedor].totalComissao += comissao;
    });

    return Object.keys(report).map((vendedor) => ({
      vendedor,
      totalVendas: report[vendedor].totalVendas,
      totalComissao: report[vendedor].totalComissao,
    }));
  }
}
